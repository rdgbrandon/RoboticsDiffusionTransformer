...
# Store all observations from all episodes
all_observations = []
...
for episode in tqdm.trange(total_episodes):
    ...
    while global_steps < MAX_EPISODE_STEPS and not done:
        ...
        preprocessed = policy.preprocess_inputs(proprio, images, text_embed)
        observations.append(preprocessed.cpu())
        actions_raw = policy.predict_action(
        ...
        actions = policy._unformat_action_to_joint(actions).to(torch.float32).squeeze(0).cpu().numpy()
    ...
    # Store observations for this episode
...
# Save all observations to a pickle file
observations_filename = f'observations_{env_id}_seed{seed}_n{total_episodes}.pkl'
with open(observations_filename, 'wb') as f:
    pkl.dump(all_observations, f)
print(f"Saved observations to {observations_filename}")
